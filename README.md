...
## relations.js で定義している関数

カテゴリを使用して関係を処理する関数を定義しています。

### searchRel()
#### 説明

リレーションを指定されたカテゴリ分類でフィルタし、フィルタされたリレーションで辿れるエンティティを列挙します。

#### 引数

| No. | 引数 | 説明 | 備考 |
| --: | ---- | --- | ---- |
|  1 | rels | 関係のリスト | (読み込むリレーションのデータを指定する) |
|  2 | colEnt1 | entity1のカラム名 |  (リレーションのデータのカラム名の情報) |
|  3 | colEnt2 | entity2のカラム名 |  (リレーションのデータのカラム名の情報) |
|  4 | colRel | 関係のカラム名 |  (リレーションのデータのカラム名の情報) |
|  5 | specifiedRel | 辿るリレーション名 | (リレーションの検索方法の指定) |
|  6 | categories | 関係のカテゴリ体系のリスト | (リレーションの検索方法の指定) |
|  7 | isTransitive | リレーションの推移性の有無 (true, false) |  (リレーションの検索方法の指定) |
|  8 | isSymmetric | リレーションの対称性の有無 (true, false) |  (リレーションの検索方法の指定) |
|  9 | isReflexive | リレーションの反射性の有無 (true, false) |  (リレーションの検索方法の指定) |
| 10 | start | 開始name | (開始, 探索対象のエンティティ) |
| 11 | goal | 終了name | (nullを指定すると、ゴールは決めずにすべて列挙する) |
...
(すぐ触ってみるためには https://novisoftware.github.io/demo/HierarchicalCategories/table-app/ にアクセスしてください)
